...
/***********************************************************************************************/
/***********************************************************************************************/
/* THE STORE PROCEDURE Backlog_Amount_SP_Full_Load WILL COMPLETE THE FOLLOWING TASKS:
...
6. COPY TEMPORARY TABLE #Snapshot_Message WITH ONLY THE MINIMUM FIELDS AND ROWS TO A NEW TABLE - #Esc_Customer_Order_Line_Bookings. THIS STEP WILL REMOVE REDUNDANT ROWS AND COMBINE ROWS, THE FINAL RESULT CAN BE REGARDED AS THE INTERMEDIATE TABLE FOR INTERMEDIATE TABLE. (Task 3)
7. CREATE A NEW BOOKING INTERMEDIATE TABLE - IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS WITH MINIMUM COLUMNS & Backlog_amount & Revenue_amount (Task 4)
8. CREATE A NEW BOOKING FACT TABLE - IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS (Task 5) 
9. CREATE A NEW TABLE - EXCEPTIONS to CONTAIN ALL THE ORDER_NO THAT NEED RAISE ATTENTION FROM BUSINESS TO CORRECT THE PRACTICE (Task6)
*/

/***********************************************************************************************/
/***********************************************************************************************/
/***********************************************************************************************/
/* Beginning of Backlog_Amount_SP_Full_Load */
CREATE PROCEDURE [dbo].[Backlog_Amount_SP_Full_Load] (@PO NVARCHAR(20))
AS     
BEGIN
...
/***********************************************************************************************/
/***********************************************************************************************/
/* ONLY THE ORDER_NO WHO HAS NO ERRORS WILL BE FURTHER PROCESSED AND UPDATE SNAPSHOT & HEADER/LINE HISTORY TABLES ACCORDINGLY, THEN WRITE TO INTERMEDIATE TABLE */
...
/*********** BEGIN OF TASK 2 - D.update INCOMPLETE_ORDER table - REMOVE WHOLE ORDER_NO IF IT IS PROCESSED */
            ...
            IF OBJECT_ID('DBO.INCOMPLETE_ORDER') IS NOT NULL                  
			begin
				    DELETE 
					FROM INCOMPLETE_ORDER
					WHERE ORDER_NO=@PO           
...
/***********************************************************************************************/
/*                                                                                                                                                  */  
/*   Task3: CREATE A NEW TEMP TABLE FROM #Snapshot_Message INCLUDING ONLY THE LEAST COLUMNS OF INSERT_FOR_REVERSAL ,BACKLOG_AMOUNT, REVENUE_AMOUNTN. 
...
CREATE TABLE #Esc_Customer_Order_Line_Bookings (
   ...
   SELECT 
    ...
    WHERE 
		  ...
		  OR (PRE_ORDER_STATE IN ('BACKLOG','PLANNED') AND LINE_OBJSTATE='INVOICED' AND QTY_INVOICED<>PRE_QTY_INVOICED AND NET_AMOUNT_BASE=0  )   /*IF NET_AMOUNT_BASE =0, THIS LINE WAS INVOICED AS 0. BUT INTERMEDIATE TABLE SHALL SHOW THE INVOICE AMOUNT INSTEAD OF PLANNED AMOUNT, EVEN THOUGH THE REVENUE_AMOUNT =0 */
          OR (ORDER_STATE ='BACKLOG' AND PRE_ORDER_STATE='PLANNED' AND NET_AMOUNT_BASE=0) /*IF NET_AMOUNT_BASE =0, THIS LINE WAS RELEASED AS 0. BUT INTERMEDIATE TABLE SHALL SHOW THE RELEASED AMOUNT INSTEAD OF PLANNED AMOUNT, EVEN THOUGH THE REVENUE_AMOUNT =0 */
...
/*DELETE THE DATA WHEN THE SALESMAN HAS TOTAL_BACKLOG_AMOUNT_SALESMAN=0 & TOTAL_REVENUE_AMOUNT_SALESMAN=0 BUT TOTAL_BACKLOG_AMOUNT_DAILY <>0 OR TOTAL_REVENUE_AMOUTN_DAILY<>0 IN THE SAME DAY AFTER THE LINE IS RELEASED */
	DELETE 
	FROM #Esc_Customer_Order_Line_Bookings
	WHERE  HISTORY_NO_RANK_SALESMAN=1
...
/*DELETE THE DATA WHEN THE SALESMAN HAS TOTAL_BACKLOG_AMOUNT_SALESMAN=0 & TOTAL_REVENUE_AMOUNT_SALESMAN=0 AND TOTAL_BACKLOG_AMOUNT_DAILY =0 & TOTAL_REVENUE_AMOUTN_DAILY=0 IN THE SAME DAY AFTER THE LINE IS RELEASED */
	DELETE 
	FROM #Esc_Customer_Order_Line_Bookings
	WHERE  HISTORY_NO_RANK_SALESMAN=1
	       AND (TOTAL_BACKLOG_AMOUNT_SALESMAN=0 AND TOTAL_REVENUE_AMOUNT_SALESMAN=0)
...
/* 12-12-2019 ADDED: TO DELETE THE RECORD WHOSE NET_AMOUNT_BASE=0 AND THIS LINE WAS CANCELLED IF ORDER WAS RELEASED IN THE SAME DAY SO IT WILL NOT BE INSERTED TO INTERMEDIATE TABLE  */
/* DELETE THE ROW WHOSE OBJSTATE='CANCELLED' OR LINE_OBJSTATE='CANCELLED' , NET_AMOUNT_BASE=0 AND THERE IS ONLY ONE ROW IN THIS TEMP TABLE - THIS LINE WAS CANCELLED IF ORDER WAS RELEASED IN THE SAME DAY SO IT WILL NOT BE INSERTED TO INTERMEDIATE TABLE*/
	DELETE
	FROM #Esc_Customer_Order_Line_Bookings
...
/***********************************************************************************************/
            /*                                                                                                           */
	        /*TASK 4: CREATE A NEW INTERMEDIATE TABLE INTERMED_CUSTOMER_ORDER_lINE_BOOKINGS  (ONLY FOR CHECKING PURPOSE) */
            /*                                                                                                           */
...
IF OBJECT_ID('IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS') IS NULL
     BEGIN
						 ...
						 (
		                        /* ID nvarchar(35) NOT NULL PRIMARY KEY IDENTITY(1,1),  */ /* ORIGINAL DATATYPE IS nvarchar(33), AND ORDER_NO + LINE_NO + REL_NO + LINE_ITEM_NO */
								...
								[ORDER_STATE] [nvarchar](20) NULL,          /*ADDED TO INTERMEDIATE TABLE*/
...
/***********************************************************************************************/
            /*                                                                    */
	        /*TASK 5: CREATE A NEW FACT TABLE INTERMED_CUSTOMER_ORDER_lINE_BOOKINGS   */
            /*                                                                    */
     ...
     BEGIN
                     ...
                     SELECT  
								 CONCAT(B.[ORDER_NO],'^',B.[LINE_NO],'^',B.[REL_NO],'^',CAST(CAST(B.[LINE_ITEM_NO] AS INT) AS NVARCHAR(12)),'^',CAST(B.[HISTORY_NO] AS NVARCHAR(12)),'^',CAST(B.[INSERT_FOR_REVERSAL] AS NVARCHAR(1)))           -- [LINE_ID]   
								  ...
								  ,S.[QTY_RETURNED]
								  ,S.[QTY_INVOICED]
								  ,B.[TOTAL_BACKLOG_AMOUNT_SALESMAN]   --USE B.[TOTAL_BACKLOG_AMOUNT_SALESMAN] NOT [BACKLOG_AMOUNT]             
								  --,B.[BACKLOG_BALANCE]
								  ,B.[TOTAL_REVENUE_AMOUNT_SALESMAN]   --USE B.[TOTAL_REVENUE_AMOUNT_SALESMAN] NOT [REVENUE_AMOUNT]
								  --,B.[REVENUE_BALANCE]
...
END           /* End of Backlog_Amount_SP_Full_Load */
