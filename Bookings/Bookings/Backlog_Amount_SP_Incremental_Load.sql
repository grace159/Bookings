...
/* 
THE STORE PROCEDURE Backlog_Amount_SP_Incremental_Load WILL COMPLETE THE FOLLOWING TASKS:
1. CREATE A TEMPORARY TABLE #Snapshot_Message FROM SNAPSHOT TABLE TO PERFORM THE CALCULATION OF BACKLOG_AMOUNT & REVENUE_AMOUNT FOR A ORDER_NO (Task 1- A). IF ORDER STATE HAS NOT 'PLANNED', THE WHOLE ORDER WILL BE SENT TO TABLE 'INCOMPLETE_DATA', AND NO FURTHER PROCESS.(Task 1- B)
2. UPDATE ORDER_STATE & PRE_ORDER_STATE, LINE_STATE & PRE_LINE_STATE. IT IS THE BASE CONDITITON FOR EVALUATION OF ACTION. (Task 1- C)
3. FOR A ORDER WITH COMPLETE DATA(BEFORE & AFTER 'RELEASED'), BACKLOG_AMOUNT & REVENUE_AMOUNT WILL BE CALCULATED FOR EACH ROW. (Task 1- D)
4. IF THERE IS ANY BUSINESS LOGICAL ERROR, THE WHOLE ORDER_NO WILL BE COPIED TO THE ERROR_HANDLING_BOOKING TABLE. NO FURTHER PROCESS WILL HAPPEN. (Task 1- E)
5. (Task 2 ) IF NO ERROR FOR THE WHOLE ORDER_NO, UPDATE the snapshot table AND two history tables WITH REMOVED_FROM_SOURCE=1 (Task 2 - A, B & C); UPDATE Incomplete_order table BY DELETE ORDER_NO WHICH IS PROCESSED (Task 2 - D)
6. COPY TEMPORARY TABLE #Snapshot_Message WITH ONLY THE MINIMUM FIELDS AND ROWS TO A NEW TABLE - #Esc_Customer_Order_Line_Bookings. THIS STEP WILL REMOVE REDUNDANT ROWS AND COMBINE ROWS (Task 3)
7. UPDATE INTERMEDIATE TABLE IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS BASED ON #Esc_Customer_Order_Line_Bookings (Task 4)- INSERT THE NON MATCHED RECORDS WHICH ARE IN TEMP TABLE TO INTERMEDIATE TABLE AND DELETE THE NON MATCHED RECORDS WHICH ARE IN INTERMEDIATE TABLE BY UPDATING ROW_ACTION=2, AND UPDATE ROW_ACTION=1 FOR THE DATA MATCH. 
8. UPDATE FACT TABLE IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS BASED ON #Esc_Customer_Order_Line_Bookings (Task 4)- INSERT THE NON MATCHED RECORDS WHICH ARE IN TEMP TABLE TO INTERMEDIATE TABLE AND DELETE THE NON MATCHED RECORDS WHICH ARE IN INTERMEDIATE TABLE BY UPDATING ROW_ACTION=2, AND UPDATE ROW_ACTION=1 FOR THE DATA MATCH. 
9. THIS SP IS USED FOR INCREMENTAL LOAD PURPOSE FOR A ORDER_NO, AND THE DATA LOADED TO SNAPSHOT TABLE SHALL BE CONTINUOUS, DATA IN SNAPSHOT SHALL BE THE COMPLETE DATA FOR THAT ORDER_NO AT THE MOMENT OF SITE_LOCAL_DATETIME.
10. SEND ORDER_NO TO TABLE - EXCEPTIONS IF THE ORDER_NO NEED RAISE ATTENTION FROM BUSINESS TO CORRECT THE PRACTICE (Task6)
*/
/***********************************************************************************************/
/***********************************************************************************************/
/***********************************************************************************************/
/* Beginning of Backlog_Amount_SP_Incremental_Load */
CREATE PROCEDURE [dbo].[Backlog_Amount_SP_Incremental_Load] (@PO NVARCHAR(20))
AS     
BEGIN
...
IF EXISTS ( SELECT HISTORY_NO FROM #SNAPSHOT_MESSAGE WHERE WAS_ORDER_PLANNED ='N' ) 
BEGIN
		IF OBJECT_ID('INCOMPLETE_ORDER') IS NULL
			  BEGIN
				  ...
				  SELECT 
				        DATA_SOURCE , OBJVERSION , HISTORY_NO , PRE_HISTORY_NO  
						,ORDER_NO ,  SITE_LOCAL_DATETIME ,PRE_SITE_LOCAL_DATETIME ,NEXT_SITE_LOCAL_DATETIME , BOOKING_DATE , LINE_NO  , REL_NO , LINE_ITEM_NO , SALESMAN_CODE ,PRE_SALESMAN_CODE , MARKET_CODE , PRE_MARKET_CODE ,REGION_CODE , PRE_REGION_CODE 
						,DISTRICT_CODE , PRE_DISTRICT_CODE , PLANNED_DELIVERY_DATE ,PRE_PLANNED_DELIVERY_DATE , BUY_QTY_DUE , PRE_BUY_QTY_DUE  
						,QTY_INVOICED , PRE_QTY_INVOICED , PPRE_QTY_INVOICED ,OBJSTATE , PRE_OBJSTATE ,LINE_OBJSTATE ,PRE_LINE_OBJSTATE 
						,NET_AMOUNT_BASE , PRE_NET_AMOUNT_BASE ,LINE_HISTORY_ORDER , ORDER_STATE , PRE_ORDER_STATE ,LINE_STATE ,PRE_LINE_STATE ,WAS_ORDER_PLANNED
						,INSERT_FOR_REVERSAL  ,BACKLOG_AMOUNT ,REVENUE_AMOUNT 
		...
		ELSE
			 BEGIN
			     ...
			     /*DELETE THE WHOLE ORDER FIRST IF IMCOMPLETE_ORDER TABLE HAS THE SAME ORDER_NO. THE TABLE INCOMPLETE_ORDER CANNOT HAVE DULPICATE DATA */
				 DELETE 
				 FROM INCOMPLETE_ORDER
				 WHERE ORDER_NO=@PO

			      /*INSERT THE ORDER_NO TO IMCOMPLETE_ORDER*/
				  INSERT INTO INCOMPLETE_ORDER
				  SELECT 
				        DATA_SOURCE , OBJVERSION , HISTORY_NO , PRE_HISTORY_NO  
						,ORDER_NO ,  SITE_LOCAL_DATETIME ,PRE_SITE_LOCAL_DATETIME ,NEXT_SITE_LOCAL_DATETIME , BOOKING_DATE , LINE_NO  , REL_NO , LINE_ITEM_NO , SALESMAN_CODE ,PRE_SALESMAN_CODE , MARKET_CODE , PRE_MARKET_CODE ,REGION_CODE , PRE_REGION_CODE 
	   ...
	   IF THE PRIOR RECORD'S DATE_SOURCE IS 'HEADER', ONLY THE RECORD WHOSE PRIOR RECORD'S LINE_NO & LINE_ITEM_NO MATCH THIS RECORD WILL BE INSERTED */  
            ...
            INNER JOIN
			                     (  								    
									...
									SELECT PRE_HISTORY_NO, LINE_NO, REL_NO, LINE_ITEM_NO, SITE_LOCAL_DATETIME,  OBJSTATE,ORDER_STATE,LINE_OBJSTATE,LINE_STATE,NET_AMOUNT_BASE                                   /*12-13-2019 ADD: OBJSTATE,ORDER_STATE,LINE_OBJSTATE,LINE_STATE,NET_AMOUNT_BASE; AND IT WILL BE USED IN THE COLUMNS ABOVE*/
									FROM   #Snapshot_Message
									WHERE  LINE_STATE IN ('A_N_CHANGED','INVOICED_A_N_CHANGED','CANCELLED_A_N')                                           /*12-13-2019 ADD: 'CANCELLED_ATT','CANCELLED_A_N'*/
									       AND INSERT_FOR_REVERSAL=0 
						                   AND ORDER_STATE NOT IN ('PLANNED') AND PRE_ORDER_STATE NOT IN ('PLANNED','CANCELLED') AND PRE_LINE_STATE NOT IN ('CANCELLED','CANCELLED_ATT','CANCELLED_A_N')       /*12-12-2019 AND LINE_OBJSTATE<>'CANCELLED' AND PRE_LINE_OBJSTATE NOT IN ('CANCELLED','CANCELLED_ATT','CANCELLED_A_N') FOR PO 'P395925' */										   	
										   AND NET_AMOUNT_BASE=0 AND PRE_NET_AMOUNT_BASE<>0		   			
...
--SELECT * FROM #Snapshot_Message
                   ...
                   /*                                                                                                        */
                   /* E: COPY THE WHOLE ORDER_NO WHO HAS LOGIC ERROR TO A ERROR_HANDLING_BOOKING TABLE FOR  REVIW AND CHECK  */
                   /*                                                                                                        */
IF EXISTS (               SELECT HISTORY_NO FROM #SNAPSHOT_MESSAGE SM WHERE  SM.BACKLOG_AMOUNT=-1  AND SM.REVENUE_AMOUNT=-1       )      /*BUSINESS LOGIC ERROR*/
                                                                                                                                                      															                  
BEGIN
		IF OBJECT_ID('ERROR_HANDLING_BOOKING') IS NULL
			  BEGIN
				   CREATE TABLE ERROR_HANDLING_BOOKING
				   (
						DATA_SOURCE nvarchar(10), OBJVERSION nvarchar (2000), HISTORY_NO BIGINT, PRE_HISTORY_NO BIGINT
						,ORDER_NO nvarchar(12),  SITE_LOCAL_DATETIME datetime,PRE_SITE_LOCAL_DATETIME datetime,NEXT_SITE_LOCAL_DATETIME datetime, BOOKING_DATE date, LINE_NO nvarchar(4) , REL_NO nvarchar(4), LINE_ITEM_NO numeric(38,4), SALESMAN_CODE nvarchar(20),PRE_SALESMAN_CODE nvarchar(20), MARKET_CODE nvarchar(10), PRE_MARKET_CODE nvarchar(10),REGION_CODE nvarchar(10), PRE_REGION_CODE nvarchar(10)
						,DISTRICT_CODE nvarchar(10), PRE_DISTRICT_CODE nvarchar(10), PLANNED_DELIVERY_DATE DATETIME, PRE_PLANNED_DELIVERY_DATE DATETIME, BUY_QTY_DUE numeric(38,4), PRE_BUY_QTY_DUE numeric(38,4)
				  ...
				  INSERT INTO ERROR_HANDLING_BOOKING
				  SELECT 
						 DATA_SOURCE , OBJVERSION , HISTORY_NO , PRE_HISTORY_NO
						,ORDER_NO ,  SITE_LOCAL_DATETIME ,PRE_SITE_LOCAL_DATETIME ,NEXT_SITE_LOCAL_DATETIME , BOOKING_DATE , LINE_NO  , REL_NO , LINE_ITEM_NO , SALESMAN_CODE ,PRE_SALESMAN_CODE , MARKET_CODE , PRE_MARKET_CODE ,REGION_CODE , PRE_REGION_CODE 
						,DISTRICT_CODE , PRE_DISTRICT_CODE ,PLANNED_DELIVERY_DATE DATETIME, PRE_PLANNED_DELIVERY_DATE DATETIME , BUY_QTY_DUE , PRE_BUY_QTY_DUE 
						,QTY_INVOICED , PRE_QTY_INVOICED , PPRE_QTY_INVOICED ,OBJSTATE , PRE_OBJSTATE ,LINE_OBJSTATE ,PRE_LINE_OBJSTATE
						,NET_AMOUNT_BASE , PRE_NET_AMOUNT_BASE ,LINE_HISTORY_ORDER , ORDER_STATE , PRE_ORDER_STATE ,LINE_STATE ,PRE_LINE_STATE ,WAS_ORDER_PLANNED 
						,INSERT_FOR_REVERSAL ,BACKLOG_AMOUNT , REVENUE_AMOUNT 
...
END
                /* END OF E: COPY THE ROW WITH LOGIC BUG TO A ERROR_HANDLING FILE FOR  REVIW AND CHECK */				
/*END OF TASK 1*/
...
/***********************************************************************************************/
/***********************************************************************************************/
/* ONLY THE ORDER_NO WHO HAS NO ERRORS WILL BE FURTHER PROCESSED AND UPDATE SNAPSHOT & HEADER/LINE HISTORY TABLES ACCORDINGLY, THEN WRITE TO FACT TABLE */

IF  NOT EXISTS ( SELECT HISTORY_NO FROM #SNAPSHOT_MESSAGE SM WHERE SM.BACKLOG_AMOUNT=-1 AND SM.REVENUE_AMOUNT=-1 ) 
BEGIN

/***********************************************************************************************/
/***********************************************************************************************/
               /*    Task2: UPDATE history and snapshot table with REMOVED_FROM_SOURCE=1 based on Temporary table #Snapshot_Message */

/* CREATE NEW UPDATE TABLE WITH COLUMN OF 'REMOVED_FROM_SOURCE'. */

/* IN  HH, LH, AND SNAPSHOT TABLES WITH COLUMN OF 'REMOVED_FROM_SOURCE': 
 WHEN IT IS 1 MEANS THIS ROW IS PROCESSED, READY TO ARCHIVE; 
 WHEN IT IS 0 MEANS THIS ROW IS NOT PROCESSED BUT THIS ORDER WAS PROCESSED (THE ROW NOT FOUND IN SNAPSHOT TABLE BUT IN HEADER/LINE HISTORY TABLE);
 WHEN IT IS NULL MEANS THIS ORDER WAS NOT PROCESSED.
*/


/* BEGIN OF TASK 2 - A. update Snapshot table by set REMOVED_FROM_SOURCE=1  */
     /*for test only*/
     --select count(*) as Snapshot_count_prior_change from RAW_ORDER_LINE_SNAPSHOT where REMOVED_FROM_SOURCE=1  and ORDER_NO=@PO
	 ...
	 UPDATE  RAW_ORDER_LINE_SNAPSHOT
			 SET REMOVED_FROM_SOURCE=0          /* IT IS NOT PROCESSED AND BUSINESS LOGIC IS CORRECT  */
			 FROM RAW_ORDER_LINE_SNAPSHOT S
			 LEFT JOIN #SNAPSHOT_MESSAGE SM
			 ON S.HISTORY_NO=SM.HISTORY_NO
			 WHERE  S.ORDER_NO=@PO
			        AND SM.HISTORY_NO IS NULL
		  
    /*for test only*/
	--select count(*) as Snapshot_count_after_change_1 from RAW_ORDER_LINE_SNAPSHOT where REMOVED_FROM_SOURCE=1  and ORDER_NO=@PO

/* END OF TASK 2 - A. update Snapshot table by set REMOVED_FROM_SOURCE=1  */



/******* BEGIN OF TASK 2 - B. update RAW_CUSTOMER_ORDER_HISTORY table */
     /*for test only*/
	 --select count(*) as HH_count_prior_change from RAW_CUSTOMER_ORDER_HISTORY where REMOVED_FROM_SOURCE=1  and ORDER_NO=@PO

     UPDATE RAW_CUSTOMER_ORDER_HISTORY
			SET PROCESS_FLAG=1                 /* IT IS PROCESSED AND BUSINESS LOGIC IS CORRECT  */
			WHERE HISTORY_NO IN ( SELECT HISTORY_NO FROM RAW_ORDER_LINE_SNAPSHOT WHERE ORDER_NO= @PO) 

     UPDATE RAW_CUSTOMER_ORDER_HISTORY
			SET PROCESS_FLAG=0                 /* IT IS NOT PROCESSED AND BUSINESS LOGIC IS CORRECT  */
			WHERE     ORDER_NO= @PO 
			      AND HISTORY_NO NOT IN ( SELECT HISTORY_NO FROM RAW_ORDER_LINE_SNAPSHOT WHERE ORDER_NO= @PO) 

     /*for test only*/
	 --select count(*) HH_count_after_change_1 from RAW_CUSTOMER_ORDER_HISTORY where REMOVED_FROM_SOURCE=1  and ORDER_NO=@PO
     --select count(*) HH_count_after_change_0 from RAW_CUSTOMER_ORDER_HISTORY where REMOVED_FROM_SOURCE=0  and ORDER_NO=@PO
	        
/******* END OF TASK 2 - B. update RAW_CUSTOMER_ORDER_HISTORY table */

            
/* BEGIN OF TASK 2 - C. update RAW_CUSTOMER_ORDER_LINE_HIST table */

    /*for test only*/
    --select count(*) as LH_count_before_change from RAW_CUSTOMER_ORDER_LINE_HIST where REMOVED_FROM_SOURCE=1 and ORDER_NO=@PO

	 UPDATE RAW_CUSTOMER_ORDER_LINE_HIST
			SET PROCESS_FLAG=1                 /* IT IS PROCESSED AND BUSINESS LOGIC IS CORRECT  */
			WHERE HISTORY_NO IN ( SELECT HISTORY_NO FROM RAW_ORDER_LINE_SNAPSHOT WHERE ORDER_NO= @PO) 
     ...
     /*for test only*/
	 --select count(*) as LH_count_after_change_1 from RAW_CUSTOMER_ORDER_LINE_HIST where REMOVED_FROM_SOURCE=1 and ORDER_NO=@PO
	 --select count(*) as LH_count_after_change_0 from RAW_CUSTOMER_ORDER_LINE_HIST where REMOVED_FROM_SOURCE=0 and ORDER_NO=@PO


/* END OF TASK 2 - C. update RAW_CUSTOMER_ORDER_LINE_HIST table */




/*********** BEGIN OF TASK 2 - D.update INCOMPLETE_ORDER table - REMOVE WHOLE ORDER_NO IF IT IS PROCESSED */
    /*for test only*/
	--select count(*) as Incomplete_order_Prior_Change from INCOMPLETE_ORDER where ORDER_NO=@PO
     ...
     /*for test only*/
     --select count(*) as Incomplete_order_after_Change from INCOMPLETE_ORDER where ORDER_NO=@PO

/*********** END OF TASK 2 - D.update INCOMPLETE_ORDER table - REMOVE WHOLE ORDER_NO IF IT IS PROCESSED */
...
END    /*END OF CREATE & INSERT INTO #Esc_Customer_Order_Line_Bookings, THEN REMOVE NOISE DATA  */
...
/***********************************************************************************************/
            /*                                                                                */
	        /*TASK 4: CREATE A NEW INTERMEDIATE TABLE INTERMED_CUSTOMER_ORDER_lINE_BOOKINGS   */
            /*                                                                                */

/*BEGIN OF TASK 4*/
IF OBJECT_ID('IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS') IS NULL
     BEGIN
						 CREATE TABLE  IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS 
						 (
                       ...
                       INSERT INTO IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS
       ...
       END    /*END OF CREATE & & INSERT INTO INTERMEDIDATE TABLE  */
ELSE
       BEGIN    /*INTERMEDIDATE TABLE EXISTS*/
					   ...
					   /*BEFORE INSERT*/
					   /*for test only*/
					   --SELECT COUNT(*) as InterMed_count_prior_insert FROM IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS WHERE ORDER_NO=@PO
                       --SELECT * FROM IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS WHERE ORDER_NO=@PO

					   /*9-18-2019 IF NO RECORD FOUND IN INTERMEDIATE TABLE */
					    
					   /* CASE 1: INTERMEDIATE TABLE DO NOT HAVE THIS ORDER_NO AND TEMP TABLE IS NULL, NOTHING HAPPENS*/

                       /* CASE 2: INTERMEDIATE TABLE DO NOT HAVE THIS ORDER_NO AND TEMP TABLE IS NOT NULL - INSERT WHOLE TEMP TABLE TO INTERMEDIATE TABLE*/
					   IF NOT EXISTS (
					                  SELECT HISTORY_NO
					                  FROM IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS 
									  WHERE ORDER_NO=@PO
					                  ) 
							   AND EXISTS (
							                  SELECT HISTORY_NO
							   ...
							   BEGIN
									   ...
									   /*INSERT THE WHOLE TEMP TABLE TO INTERMEDIATE TABLE*/
									   INSERT INTO IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS
												   (DATA_SOURCE, OBJVERSION, HISTORY_NO, ORDER_NO, LINE_NO,REL_NO,LINE_ITEM_NO,SITE_LOCAL_DATETIME,BOOKING_DATE,SALESMAN_CODE,MARKET_CODE,REGION_CODE,DISTRICT_CODE, PLANNED_DELIVERY_DATE, OBJSTATE,ORDER_STATE, LINE_OBJSTATE, INSERT_FOR_REVERSAL, BUY_QTY_DUE, QTY_INVOICED, NET_AMOUNT_BASE, BACKLOG_AMOUNT,REVENUE_AMOUNT, BACKLOG_BALANCE, REVENUE_BALANCE, ROW_ACTION) 
									   SELECT  DATA_SOURCE, OBJVERSION, HISTORY_NO, ORDER_NO, LINE_NO,REL_NO,LINE_ITEM_NO,SITE_LOCAL_DATETIME,BOOKING_DATE,SALESMAN_CODE,MARKET_CODE,REGION_CODE,DISTRICT_CODE, PLANNED_DELIVERY_DATE, OBJSTATE,ORDER_STATE, LINE_OBJSTATE, INSERT_FOR_REVERSAL, BUY_QTY_DUE, QTY_INVOICED, NET_AMOUNT_BASE,TOTAL_BACKLOG_AMOUNT_SALESMAN,TOTAL_REVENUE_AMOUNT_SALESMAN, BACKLOG_BALANCE, REVENUE_BALANCE,1
									   FROM #Esc_Customer_Order_Line_Bookings				  
									   ORDER BY LINE_NO, REL_NO, LINE_ITEM_NO, SITE_LOCAL_DATETIME
                        ...
                        ELSE   /*9-18-2019 IF RECORDS FOUND IN INTERMEDIATE TABLE */
						       BEGIN

							      /*CASE 3 : INTERMEDIATE TABLE HAS THIS ORDER_NO AND TEMP TABLE IS NULL - INTERMEDIATE TABLE RECORDS NEED TO BE DELETED LATER AFTER UPDATING ROW_ACTION=2 */                					     
                                       /*DELETE THE ROW FROM INTERMEDIATE TABLE IF TMEP TABLE IS EMPTY*/
                                       UPDATE IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS 
											  SET ROW_ACTION=2	
									   WHERE  ORDER_NO=@PO
									          AND NOT EXISTS (SELECT HISTORY_NO FROM #Esc_Customer_Order_Line_Bookings)

								  /*CASE 4 : INTERMEDIATE TABLE HAS THIS ORDER_NO AND TEMP TABLE IS NOT NULL - DELETE NON MATCHED RECORDS WHICH ARE IN INTERMEDIATE TABLE (TEMP2_CTE) , 
								                                                                     - INSERT NON MATCHED RECORDS WHICH ARE IN TEMP TABLE TO INTERMEDIATE TABLE (TEMP1_CTE)
																									 - KEEP MATCHED RECORDS BY UPDATING ROW_ACTION=1 (TEMP3_CTE) */
									   
									   /* TEMP1_CTE - FIND THE DATA WHICH IS NOT IN INTERMEDIATE TABLE BUT IN TEMP TABLE ONLY */					  					   					  					     	                   
									   ;WITH TEMP1_CTE
									   AS             
													(
														 SELECT  T.* /* , F.HISTORY_NO AS HIS_NO, F.LINE_NO AS L_NO, F.REL_NO AS RL_NO, F.LINE_ITEM_NO AS LIN_NO */
										   										           
														 FROM #Esc_Customer_Order_Line_Bookings T 
														 LEFT JOIN IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS AS F
														 ON T.ORDER_NO=F.ORDER_NO
															AND T.LINE_NO=F.LINE_NO
															AND T.REL_NO=F.REL_NO
															AND T.LINE_ITEM_NO=F.LINE_ITEM_NO
															AND T.HISTORY_NO=F.HISTORY_NO
															AND T.INSERT_FOR_REVERSAL=F.INSERT_FOR_REVERSAL
														 WHERE F.HISTORY_NO IS NULL																							
													  )

                                       /*SELECT * FROM TEMP1_CTE */
                                      
									   /*9-18-2019 INSERT TO INTERMEDIATE TABLE - TEMP1_CTE IS THE UNIQUE DATA OF TEMP TABLE */					   
									   INSERT INTO IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS
											  (DATA_SOURCE, OBJVERSION, HISTORY_NO, ORDER_NO, LINE_NO,REL_NO,LINE_ITEM_NO,SITE_LOCAL_DATETIME,BOOKING_DATE,SALESMAN_CODE,MARKET_CODE,REGION_CODE,DISTRICT_CODE, PLANNED_DELIVERY_DATE, OBJSTATE,ORDER_STATE, LINE_OBJSTATE, INSERT_FOR_REVERSAL, BUY_QTY_DUE, QTY_INVOICED, NET_AMOUNT_BASE,BACKLOG_AMOUNT,REVENUE_AMOUNT, BACKLOG_BALANCE, REVENUE_BALANCE, ROW_ACTION) 
									   SELECT  DATA_SOURCE, OBJVERSION, HISTORY_NO, ORDER_NO, LINE_NO,REL_NO,LINE_ITEM_NO,SITE_LOCAL_DATETIME,BOOKING_DATE,SALESMAN_CODE,MARKET_CODE,REGION_CODE,DISTRICT_CODE, PLANNED_DELIVERY_DATE, OBJSTATE,ORDER_STATE, LINE_OBJSTATE, INSERT_FOR_REVERSAL, BUY_QTY_DUE, QTY_INVOICED, NET_AMOUNT_BASE,TOTAL_BACKLOG_AMOUNT_SALESMAN,TOTAL_REVENUE_AMOUNT_SALESMAN, BACKLOG_BALANCE, REVENUE_BALANCE,  1
									   FROM TEMP1_CTE				  
									   ...
									   /*TEMP2_CTE - FIND THE DATA WHICH IS IN INTERMEDIATE TABLE ONLY BUT NOT IN TEMP TABLE FOR THIS ORDER_NO */      
									   ;WITH TEMP2_CTE
									   AS             
													(
														 SELECT  F.*  /* ,  T.HISTORY_NO AS HIS_NO, T.LINE_NO AS L_NO, T.REL_NO AS RL_NO, T.LINE_ITEM_NO AS LIN_NO	*/								           
														 FROM IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS AS F  
														 LEFT JOIN #Esc_Customer_Order_Line_Bookings T
														 ON F.ORDER_NO=T.ORDER_NO
															AND F.LINE_NO=T.LINE_NO
															AND F.REL_NO=T.REL_NO
															AND F.LINE_ITEM_NO=T.LINE_ITEM_NO
															AND F.HISTORY_NO=T.HISTORY_NO
															AND F.INSERT_FOR_REVERSAL=T.INSERT_FOR_REVERSAL
														 WHERE T.HISTORY_NO IS NULL	
														       AND F.ORDER_NO=@PO
										    	   																	
													  )
                                      /*  SELECT * FROM TEMP2_CTE  */
                                       
									   /*UPDATE INTERMEDIATE TABLE - TEMP2_CTE IS THE UNIQUE DATA OF INTERMEDIATE TABLE WHOSE ORDER_NO MATCH TEMP TABLE */					   					                       					 
									   UPDATE IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS 
											  SET ROW_ACTION=2					   
									   FROM TEMP2_CTE
									   WHERE     IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS.ORDER_NO=@PO
											 AND IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS.LINE_NO=TEMP2_CTE.LINE_NO
											 AND IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS.REL_NO=TEMP2_CTE.REL_NO
											 AND IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS.LINE_ITEM_NO=TEMP2_CTE.LINE_ITEM_NO
											 AND IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS.HISTORY_NO=TEMP2_CTE.HISTORY_NO
											 AND IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS.INSERT_FOR_REVERSAL=TEMP2_CTE.INSERT_FOR_REVERSAL
					                 
									   /*TEMP3_CTE - FIND THE DATA WHICH MATCH IN BOTH TEMP TABLE AND INTERMEDIATE TABLE FOR THIS ORDER_NO */      
									   /* NOT NEEDED
									   ;WITH TEMP3_CTE
									   AS             
													(
														 SELECT  T.*  /* ,  T.HISTORY_NO AS HIS_NO, T.LINE_NO AS L_NO, T.REL_NO AS RL_NO, T.LINE_ITEM_NO AS LIN_NO	*/								           
														 FROM  #Esc_Customer_Order_Line_Bookings T  
														 INNER JOIN IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS AS F 
														 ON T.ORDER_NO=F.ORDER_NO
															AND T.LINE_NO=F.LINE_NO
															AND T.REL_NO=F.REL_NO
															AND T.LINE_ITEM_NO=F.LINE_ITEM_NO
                                       ...
                                       /*UPDATE INTERMEDIATE TABLE - TEMP3_CTE IS THE MATCHED DATA OF INTERMEDIATE TABLE AND TEMP TABLE */										   									   
									   UPDATE IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS 
											  SET ROW_ACTION=1					   
									   FROM TEMP3_CTE
									   WHERE     IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS.ORDER_NO=@PO
											 AND IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS.LINE_NO=TEMP3_CTE.LINE_NO
											 AND IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS.REL_NO=TEMP3_CTE.REL_NO
											 AND IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS.LINE_ITEM_NO=TEMP3_CTE.LINE_ITEM_NO
											 AND IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS.HISTORY_NO=TEMP3_CTE.HISTORY_NO
											 AND IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS.INSERT_FOR_REVERSAL=TEMP3_CTE.INSERT_FOR_REVERSAL					      
                                       NOT NEEDED */
									  

					   		   END		/*END OF RECORDS FOUND IN INTERMEDIATE TABLE */   
					...
					/*for test only*/
					/*
					 SELECT COUNT(*) as InterMed_count_after_insert FROM IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS WHERE ORDER_NO=@PO

					 SELECT * FROM IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS WHERE ORDER_NO=@PO
 
					 SELECT COUNT(*) as Intermed_action_2_count FROM IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS WHERE ROW_ACTION=2 AND ORDER_NO=@PO
					 SELECT * FROM IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS WHERE ROW_ACTION=2 AND ORDER_NO=@PO
					*/

					/* DELETE IN INTERMEDIATE TABLE WHOSE ROW_ACTION=2 */
					DELETE 
					FROM [dbo].[IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS]
					WHERE ORDER_NO=@PO
						AND ROW_ACTION=2
					...
					FROM [dbo].[IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS]
					WHERE ORDER_NO=@PO
					ORDER BY LINE_NO, REL_NO, LINE_ITEM_NO, HISTORY_NO
					*/
           END   /* END OF INTERMEDIDATE TABLE EXISTS*/
...
/***********************************************************************************************/
            /*                                                                                */
	        /*TASK 5: UPDATE FACT TABLE - FACT_CUSTOMER_ORDER_lINE_BOOKINGS BY INSERT NEW RECORDS, DELETE RECORDS (RULE - ONLY ONE RECORD FOR A LINE_NO EACH DAY EXCEPT REVERSAL)*/
            /*                                                                                */

/*BEGIN OF TASK 5*/
IF OBJECT_ID('IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS') IS NULL
     BEGIN
						 CREATE TABLE  IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS 
						 (
                       ...
                       INSERT INTO IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS
					   SELECT  
					   /* IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS -
					   DATA_SOURCE,OBJVERSION, HISTORY_NO, ORDER_NO, LINE_NO,REL_NO,LINE_ITEM_NO,SITE_LOCAL_DATETIME,BOOKING_DATE,SALESMAN_CODE,MARKET_CODE,REGION_CODE,DISTRICT_CODE, PLANNED_DELIVERY_DATE, OBJSTATE,ORDER_STATE, LINE_OBJSTATE, INSERT_FOR_REVERSAL, BUY_QTY_DUE, QTY_INVOICED, NET_AMOUNT_BASE,TOTAL_BACKLOG_AMOUNT_SALESMAN,TOTAL_REVENUE_AMOUNT_SALESMAN, BACKLOG_BALANCE, REVENUE_BALANCE,1
					   */
								  ...
								  --,B.[BACKLOG_BALANCE]               -- NOT USE
								  ,B.[TOTAL_REVENUE_AMOUNT_SALESMAN]
								  --,B.[REVENUE_BALANCE]               -- NOT USE
       ...
       END             /*FACT TABLE DOES NOT EXIST*/
ELSE
       BEGIN           /*FACT TABLE EXISTS*/
	                   
	                   					            	                   
					   /*for test only*/
					  --SELECT COUNT(*) AS Fact_count_prior_insert FROM IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS WHERE ORDER_NO=@PO
                      --SELECT * FROM IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS WHERE ORDER_NO=@PO

					   /*9-18-2019 IF NO RECORD FOUND IN FACT TABLE */
					    
					   /* CASE 1: FACT TABLE DO NOT HAVE THIS ORDER_NO AND TEMP TABLE IS NULL, NOTHING HAPPENS*/

                       /* CASE 2: FACT TABLE DO NOT HAVE THIS ORDER_NO AND TEMP TABLE IS NOT NULL - INSERT WHOLE TEMP TABLE TO FACT TABLE*/
					   IF NOT EXISTS (
					                  SELECT HISTORY_NO
					                  FROM IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS 
									  WHERE ORDER_NO=@PO
					                  ) 
							   AND EXISTS (
							                  SELECT HISTORY_NO
							   ...
							   BEGIN
									   ...
									   /*INSERT THE WHOLE TEMP TABLE TO FACT TABLE*/
									   INSERT INTO IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS
												   /* IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS -
												   (DATA_SOURCE, OBJVERSION, HISTORY_NO, ORDER_NO, LINE_NO,REL_NO,LINE_ITEM_NO,SITE_LOCAL_DATETIME,BOOKING_DATE,SALESMAN_CODE,MARKET_CODE,REGION_CODE,DISTRICT_CODE, PLANNED_DELIVERY_DATE, OBJSTATE,ORDER_STATE, LINE_OBJSTATE, INSERT_FOR_REVERSAL, BUY_QTY_DUE, QTY_INVOICED, NET_AMOUNT_BASE, BACKLOG_AMOUNT,REVENUE_AMOUNT, BACKLOG_BALANCE, REVENUE_BALANCE, ROW_ACTION) 
									                */
													 ...
													 [LINE_ID]
													  ...
													  ,[DIM_SUPPLIER_ID]
													  ,[DIM_SALES_PART_ID]
													  ,[DIM_INVENTORY_PART_ID]
													  ,[DIM_ORDER_CANCEL_REASON_ID]
													  ,[DIM_SHIP_VIA_ID]
													  ,[DIM_DELIVERY_TERM_ID]
													  ,[DIM_PROJECT_ID]
													  ,[DIM_ACTIVITY_ID]
													  ,[DIM_SALES_REGION_ID]
													  ,[DIM_SALES_PERSON_ID]
													  ...
													  ,[DIM_SALES_MARKET_ID]
													  ,[DIM_CUST_ORDER_TYPE_ID]
													  ,[DIM_CONFIGURATION_ID]
													  ,[DIM_CONDITION_CODE_ID]
													  ,[CUSTOMER_PO_NO]
													  ,[CUSTOMER_NO_PAY]
													  ,[ROW_ACTION]
													  --,[Modified_By_User]
							   ...
							   END            /*9-18-2019 IF NO RECORD FOUND IN FACT TABLE */

                        ELSE   /*9-18-2019 IF RECORDS FOUND IN FACT TABLE */
						       BEGIN

							      /*CASE 3 : FACT TABLE HAS THIS ORDER_NO AND TEMP TABLE IS NULL - FACT TABLE RECORDS NEED TO BE DELETED LATER AFTER UPDATING ROW_ACTION=2 */                					     
                                       /*DELETE THE ROW FROM FACT TABLE IF TMEP TABLE IS EMPTY*/
                                       UPDATE IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS 
											  SET ROW_ACTION=2	
									   WHERE  ORDER_NO=@PO
									          AND NOT EXISTS (SELECT HISTORY_NO FROM #Esc_Customer_Order_Line_Bookings)

								  /*CASE 4 : FACT TABLE HAS THIS ORDER_NO AND TEMP TABLE IS NOT NULL - DELETE NON MATCHED RECORDS WHICH ARE IN FACT TABLE (TEMP2_CTE) , 
								                                                                     - INSERT NON MATCHED RECORDS WHICH ARE IN TEMP TABLE TO FACT TABLE (TEMP1_CTE)
																									 - KEEP MATCHED RECORDS BY UPDATING ROW_ACTION=1 (TEMP3_CTE) */
									   
									   /* TEMP1_CTE - FIND THE DATA WHICH IS NOT IN FACT TABLE BUT IN TEMP TABLE ONLY */					  					   					  					     	                   
									   ;WITH TEMP1_CTE
									   AS             
													(
														 SELECT  T.* /* , F.HISTORY_NO AS HIS_NO, F.LINE_NO AS L_NO, F.REL_NO AS RL_NO, F.LINE_ITEM_NO AS LIN_NO */
														 ...
														 FROM #Esc_Customer_Order_Line_Bookings T 
														 LEFT JOIN IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS AS F
														 ON T.ORDER_NO=F.ORDER_NO
															AND T.LINE_NO=F.LINE_NO
															AND T.REL_NO=F.REL_NO
															AND T.LINE_ITEM_NO=F.LINE_ITEM_NO
									   ...
									   /*9-18-2019 INSERT TO FACT TABLE - TEMP1_CTE IS THE UNIQUE DATA OF TEMP TABLE */					   
									   INSERT INTO IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS
											  /* IFSAppl_INTERMED_CUSTOMER_ORDER_LINE_BOOKINGS -
											  (DATA_SOURCE, OBJVERSION, HISTORY_NO, ORDER_NO, LINE_NO,REL_NO,LINE_ITEM_NO,SITE_LOCAL_DATETIME,BOOKING_DATE,SALESMAN_CODE,MARKET_CODE,REGION_CODE,DISTRICT_CODE, PLANNED_DELIVERY_DATE, OBJSTATE,ORDER_STATE, LINE_OBJSTATE, INSERT_FOR_REVERSAL, BUY_QTY_DUE, QTY_INVOICED, NET_AMOUNT_BASE,BACKLOG_AMOUNT,REVENUE_AMOUNT, BACKLOG_BALANCE, REVENUE_BALANCE, ROW_ACTION) 
									          */
									   ...
									   DATA_SOURCE, OBJVERSION, HISTORY_NO, ORDER_NO, LINE_NO,REL_NO,LINE_ITEM_NO,SITE_LOCAL_DATETIME,BOOKING_DATE,SALESMAN_CODE,MARKET_CODE,REGION_CODE,DISTRICT_CODE, PLANNED_DELIVERY_DATE, OBJSTATE,ORDER_STATE, LINE_OBJSTATE, INSERT_FOR_REVERSAL, BUY_QTY_DUE, QTY_INVOICED, NET_AMOUNT_BASE,TOTAL_BACKLOG_AMOUNT_SALESMAN,TOTAL_REVENUE_AMOUNT_SALESMAN, BACKLOG_BALANCE, REVENUE_BALANCE,  1
									   */
													  CONCAT(TEMP1_CTE.[ORDER_NO],'^',TEMP1_CTE.[LINE_NO],'^',TEMP1_CTE.[REL_NO],'^',CAST(CAST(TEMP1_CTE.[LINE_ITEM_NO] AS INT) AS NVARCHAR(12)),'^',CAST(TEMP1_CTE.[HISTORY_NO] AS NVARCHAR(12)),'^',CAST(TEMP1_CTE.[INSERT_FOR_REVERSAL] AS NVARCHAR(1)))           -- [LINE_ID]   
													  ,TEMP1_CTE.[ORDER_NO]
													  ,TEMP1_CTE.[LINE_NO]
													  ,TEMP1_CTE.[REL_NO]
													  ,TEMP1_CTE.[LINE_ITEM_NO]
													  ,TEMP1_CTE.[HISTORY_NO]
													  ,TEMP1_CTE.[INSERT_FOR_REVERSAL]
													  ,S.[BUY_QTY_DUE]                     --[SALES_QTY] IN FACT TABLE
													  ,S.[QTY_ASSIGNED]
													  ,S.[QTY_PICKED]
													  ,S.[QTY_SHORT]
													  ...
													  ,S.[QTY_RETURNED]
													  ,S.[QTY_INVOICED]
													  ,TEMP1_CTE.[TOTAL_BACKLOG_AMOUNT_SALESMAN]
													  --,TEMP1_CTE.[BACKLOG_BALANCE]
													  ,TEMP1_CTE.[TOTAL_REVENUE_AMOUNT_SALESMAN]
													  --,TEMP1_CTE.[REVENUE_BALANCE]
													  ,TEMP1_CTE.[SITE_LOCAL_DATETIME]             -- [TRANSACTION_DATE] IN FACT TABLE
													  ,S.[BASE_SALE_UNIT_PRICE]   
													  --,[BASE_SAL_PRICE_INCL_TAX]           -- NOT IN SNAPSHOT
													  ,S.[NET_AMOUNT_BASE]                 -- [NET_AMOUNT_IN_BASE]
													  --,[GROSS_AMOUNT_IN_BASE]              -- NOT IN SNAPSHOT
										...
										FROM TEMP1_CTE 
										LEFT JOIN RAW_ORDER_LINE_SNAPSHOT S
										ON TEMP1_CTE.ORDER_NO=S.ORDER_NO
										AND TEMP1_CTE.LINE_NO=S.LINE_NO
										AND TEMP1_CTE.REL_NO=S.REL_NO
										AND TEMP1_CTE.LINE_ITEM_NO=S.LINE_ITEM_NO
										AND TEMP1_CTE.HISTORY_NO=S.HISTORY_NO 

										ORDER BY LINE_NO, REL_NO, LINE_ITEM_NO, SITE_LOCAL_DATETIME
									  				                   
					   
									   /*TEMP2_CTE - FIND THE DATA WHICH IS IN FACT TABLE ONLY BUT NOT IN TEMP TABLE FOR THIS ORDER_NO */      
									   ;WITH TEMP2_CTE
									   AS             
													(
														 SELECT  F.*  /* ,  T.HISTORY_NO AS HIS_NO, T.LINE_NO AS L_NO, T.REL_NO AS RL_NO, T.LINE_ITEM_NO AS LIN_NO	*/								           
														 FROM IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS AS F  
														 LEFT JOIN #Esc_Customer_Order_Line_Bookings T
														 ON F.ORDER_NO=T.ORDER_NO
															AND F.LINE_NO=T.LINE_NO
															AND F.REL_NO=T.REL_NO
                                      ...
                                      /*  SELECT * FROM TEMP2_CTE  */
									   ...
									   /*UPDATE FACT TABLE - TEMP2_CTE IS THE UNIQUE DATA OF FACT TABLE WHOSE ORDER_NO MATCH TEMP TABLE */					   					                       					 
									   UPDATE IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS 
											  SET ROW_ACTION=2					   
									   FROM TEMP2_CTE
									   WHERE     IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS.ORDER_NO=@PO
											 AND IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS.LINE_NO=TEMP2_CTE.LINE_NO
											 AND IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS.REL_NO=TEMP2_CTE.REL_NO
											 AND IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS.LINE_ITEM_NO=TEMP2_CTE.LINE_ITEM_NO
											 AND IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS.HISTORY_NO=TEMP2_CTE.HISTORY_NO
											 AND IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS.INSERT_FOR_REVERSAL=TEMP2_CTE.INSERT_FOR_REVERSAL
					                 
									   /*TEMP3_CTE - FIND THE DATA WHICH MATCH IN BOTH TEMP TABLE AND FACT TABLE FOR THIS ORDER_NO */      
									   /*
									   ;WITH TEMP3_CTE
									   AS             
													(
														 SELECT  T.*  /* ,  T.HISTORY_NO AS HIS_NO, T.LINE_NO AS L_NO, T.REL_NO AS RL_NO, T.LINE_ITEM_NO AS LIN_NO	*/								           
														 FROM  #Esc_Customer_Order_Line_Bookings T  
														 INNER JOIN IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS AS F 
														 ON T.ORDER_NO=F.ORDER_NO
															AND T.LINE_NO=F.LINE_NO
															AND T.REL_NO=F.REL_NO
															AND T.LINE_ITEM_NO=F.LINE_ITEM_NO
                                       ...
                                       /*UPDATE FACT TABLE - TEMP3_CTE IS THE MATCHED DATA OF FACT TABLE AND TEMP TABLE */	
									   UPDATE IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS 
											  SET ROW_ACTION=1					   
									   FROM TEMP3_CTE
									   WHERE     IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS.ORDER_NO=@PO
											 AND IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS.LINE_NO=TEMP3_CTE.LINE_NO
											 AND IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS.REL_NO=TEMP3_CTE.REL_NO
											 AND IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS.LINE_ITEM_NO=TEMP3_CTE.LINE_ITEM_NO
											 AND IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS.HISTORY_NO=TEMP3_CTE.HISTORY_NO
											 AND IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS.INSERT_FOR_REVERSAL=TEMP3_CTE.INSERT_FOR_REVERSAL					      
                                      
									  */
					   		   END		 /*9-18-2019 IF RECORDS FOUND IN FACT TABLE */  
							...
							/*AFRER INSERT & UPDATE*/
							 /*for test only*/
							/*
							 SELECT COUNT(*) as Fact_count_after_insert FROM IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS WHERE ORDER_NO=@PO

							 SELECT * FROM IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS WHERE ORDER_NO=@PO
 
							 SELECT COUNT(*) as Fact_action_2_count FROM IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS WHERE ROW_ACTION=2 AND ORDER_NO=@PO
							 SELECT * FROM IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS WHERE ROW_ACTION=2 AND ORDER_NO=@PO
							*/


							/* DELETE IN FACT TABLE WHOSE ROW_ACTION=2 */
							DELETE 
							FROM [dbo].[IFSAppl_FACT_CUSTOMER_ORDER_LINE_BOOKINGS]
							WHERE ORDER_NO=@PO
								AND ROW_ACTION=2
							...
							ORDER BY LINE_NO, REL_NO, LINE_ITEM_NO, HISTORY_NO
							*/
                 END    /*END OF FACT TABLE EXISTS*/

/*END OF TASK 5*/


             /********************************************************************************************************/    
             /**Task6: IF SOME SITUATIONS NEED RAISE THE ATTENTION FROM BUSINESS AND LET THEM CORRECT THEIR PRACTICE**/
...
END           /* END OF 'IF NOT EXISTS ( SELECT HISTORY_NO FROM #SNAPSHOT_MESSAGE SM WHERE SM.BACKLOG_AMOUNT=-1 AND SM.REVENUE_AMOUNT=-1              /*BUSINESS LOGIC ERROR  */
                                                                                          
                                       )'   */

/***********************************************************************************************/

END           /*End of THIS ORDER WAS PLANNED BEFORE (Task1 - B)*/
END           /* End of Backlog_Amount_SP_Incremental_Load */
